(function () {
  ...
  const STUDIO_BASE_PATH = (() => {
    const pathname = window.location.pathname || '/';
    const knownBases = ['/music/MSA_D', '/MSA_D'];
    return knownBases.find((basePath) => pathname === basePath || pathname.startsWith(`${basePath}/`)) || '/MSA_D';
  })();
  ...
  function redirectToLogin() {
    window.location.replace(studioPath('login.html'));
  }
  ...
  function normalizeNavigation() {
    ...
    if (!nav) return;
    const revenueLink = Array.from(nav.querySelectorAll('.nav-link')).find((link) => {
      const href = link.getAttribute('href')?.replace(/\/+$/, '');
      return href === `${STUDIO_BASE_PATH}/dashboard/revenue` || href === '/dashboard/revenue';
    });
    if (revenueLink) {
