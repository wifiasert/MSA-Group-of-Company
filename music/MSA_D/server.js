...
function getRequestBasePath(pathname) {
  const normalizedPathname = pathname || '/';
  return PUBLIC_BASE_PATHS.find((basePath) => (
...
function normalizeRoutePathname(pathname) {
  const matchedBase = getRequestBasePath(pathname);
  if (pathname === matchedBase || pathname === `${matchedBase}/`) {
    return '/';
  }
  if (pathname.startsWith(`${matchedBase}/`)) {
    return pathname.slice(matchedBase.length) || '/';
  }
  return pathname;
}
...
function buildPublicPath(pathname, targetPath) {
  ...
  const publicBasePath = getRequestBasePath(pathname || '/');
  if (!normalizedTarget) {
  ...
  if (normalizedTarget.startsWith('dashboard/')) {
    return `${publicBasePath}/${normalizedTarget}`;
  }
...
if (require.main === module) {
  ...
  const isRootEntry = pathname === '/index.html' || pathname === '/' || PUBLIC_BASE_PATHS.some((basePath) => pathname === basePath || pathname === `${basePath}/`);
