...
const pages = [
  ...
  { title: "Dashboard", body: "Client portal, downloads and orders", url: "/dashboard/" },
...
function renderServices() {
  ...
  target.innerHTML = services.map(([name, description, price]) => `
    <article class="glass-card">
      ...
      <a class="inline-link" href="/contact/">Request a proposal</a>
    </article>
  `).join("");
}
...
function renderSoftware() {
  ...
  target.innerHTML = software.map((item) => `
    <article class="glass-card">
      ...
      <a class="secondary-button" href="/software/store/">View product</a>
    </article>
  `).join("");
}
...
function setupSearch() {
  ...
  input.addEventListener("input", (event) => {
    ...
    results.innerHTML = term ? matches.map((page) => `<a href="${page.url}" data-close><strong>${page.title}</strong><p>${page.body}</p></a>`).join("") : "";
  });
}
