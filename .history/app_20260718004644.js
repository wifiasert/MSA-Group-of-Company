...
function setupForms() {
  ...
  $$('[data-form]').forEach((form) => {
    ...
    $$('input[type="email"]', form).forEach((emailInput) => {
    ...
    const sourceValue = `${document.title} | ${window.location.pathname}`;
    hiddenField("form_name", form.dataset.form || "site_form");
    hiddenField("page_title", document.title);
    hiddenField("page_url", window.location.href);
    hiddenField("submitted_from", sourceValue);
    hiddenField("_subject", `New ${form.dataset.form || "website"} submission from ${document.title}`);
    ...
    form.addEventListener("submit", async (event) => {
      ...
      formData.set("form_name", form.dataset.form || "site_form");
